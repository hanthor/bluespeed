...
# setup-gemini-cli.sh - Configure Gemini CLI with dosu and linux-mcp-server
...
main() {
    log_info "Starting Gemini CLI setup for bluespeed-onboarding..."
    ...
    if [[ ! -f "$GEMINI_CONFIG" ]]; then
        log_info "Gemini CLI settings not found, creating default..."
        create_default_gemini_settings "$GEMINI_CONFIG" || {
    ...
    BACKUP_PATH=$(backup_file "$GEMINI_CONFIG")
    ...
    trap "cleanup_on_error '$GEMINI_CONFIG' '$BACKUP_PATH'" ERR
    
    # Merge MCP servers configuration
    log_info "Configuring MCP servers..."
    merge_gemini_mcp_servers "$GEMINI_CONFIG" "$USERNAME" || {
        status=$?
        if [[ $status -eq 2 ]]; then
    ...
    if ! validate_json "$GEMINI_CONFIG"; then
        log_error "Configuration validation failed, restoring backup..."
        restore_backup "$GEMINI_CONFIG" "$BACKUP_PATH"
        exit 1
    fi
    
    # Success!
    log_success "Gemini CLI configuration complete!"
    echo ""
    log_info "Next steps:"
    echo "  1. Restart your Gemini CLI session"
    echo "  2. Verify MCP servers loaded: Check startup messages or server list"
    echo "  3. Test: Ask 'Can you check my system information?' (tests linux-mcp-server)"
    echo ""
    log_info "Troubleshooting:"
    echo "  - Config: ~/.gemini/settings.json"
...
if [[ "${1:-}" == "--help" ]] || [[ "${1:-}" == "-h" ]]; then
    echo "Usage: $0"
    echo ""
    echo "Configure Gemini CLI with dosu MCP and linux-mcp-server for Bluefin maintenance."
    echo ""
    echo "This script:"
    echo "  - Validates prerequisites (jq, linux-mcp-server)"
    echo "  - Backs up existing Gemini CLI settings"
